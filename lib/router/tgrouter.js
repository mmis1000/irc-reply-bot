(function() {
  var LRU, Media, Message, Senter, Telegram, TelegramFile, TelegramRouter, TelegramText, TextRouter, UTF8LengthSplit, User, co, createBotMessage, createMediaFromPhotoList, createSenderFromMessage, createSenderFromUser,
        ...
        return function*() {
          ...
          if (!userPhotoMedias) {
            ...
            if (userPhoto.total_count > 0) {
              userPhotoMedias = userPhoto.photos.map(createMediaFromPhotoList).map(function(media, index) {
                media.role = 'avatar';
  ...
  createSenderFromMessage = function(message, telegramRouter) {
    return co(function*() {
      var api, cachedChannel, cachedUser, channelId, channelInfo, compareStr, compareStr1, e, error, error1, sender, userInfo, userName, userPhoto, userPhotoMedias;
  ...
  createSenderFromUser = function(user, telegramRouter) {
    return co(function*() {
      var cachedUser, channelId, compareStr, compareStr1, e, error, sender, userInfo, userName, userPhoto, userPhotoMedias;
      try {
        ...
        if (!userPhotoMedias) {
          ...
          if (userPhoto.total_count > 0) {
            userPhotoMedias = userPhoto.photos.map(createMediaFromPhotoList).map(function(media) {
              media.role = 'avatar';
              media.id = userPhoto.photos[index][0].file_id + "@telegram-avatar";
              return media;
            });
