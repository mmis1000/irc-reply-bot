(function() {
      ...
      if (message.medias.length > 0) {
        if (!message.meta["_" + (this.getRouterIdentifier())]) {
          ...
          if (originalInfo.sticker) {
            this.api.sendSticker(originalInfo.chat.id, originalInfo.sticker.file_id);
          } else if (originalInfo.photo) {
            this.api.sendPhoto(originalInfo.chat.id, originalInfo.photo[originalInfo.photo.length - 1].file_id);
          } else if (originalInfo.audio) {
            this.api.sendAudio(originalInfo.chat.id, originalInfo.audio.file_id);
          } else if (originalInfo.video) {
            this.api.sendVideo(originalInfo.chat.id, originalInfo.video.file_id);
          } else {
            (_ref1 = TextRouter.prototype.outputMessage).call.apply(_ref1, [this].concat(__slice.call(arguments)));
          }
          return;
        }
      }
  ...
  TelegramText = (function() {
    ...
    TelegramText.toHTML = function(text, entities) {
      var chars, entity, i, name, offset, realOffset, url, _i, _j, _len, _ref, _ref1;
      chars = text.split('');
