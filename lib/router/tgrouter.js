...
TelegramRouter = class TelegramRouter extends TextRouter {
  ...
  _init() {
    ...
    this.api.getMe(meHandle = (err, res) => {
      ...
      this.api.on('message', (message) => {
        var allEntry, botMessage, channelId, clonedRouter, currentProto, j, key, len, message_, message_id, ref, ref1, self, userName, value;
  ...
  output(message, to, message_id, originalChannel, nobuffer, textFormat = 'raw') {
    var message_id_temp, person, results;
...
TelegramText = class TelegramText {
  static toHTML(text, entities) {
    var chars, entity, i, name, offset, realOffset, ref, ref1, ref2, ref3, url;
    chars = text.split('');
    ...
    for (entity of entities) {
      switch (entity.type) {
        ...
        case 'pre':
          ...
          ref1 = (function() {
            var results = [];
            for (var j = realOffset, ref = realOffset + entity.length - 1; realOffset <= ref ? j <= ref : j >= ref; realOffset <= ref ? j++ : j--){ results.push(j); }
            return results;
          }).apply(this);
          for (i of ref1) {
            if (chars[i] === '<br/>') {
