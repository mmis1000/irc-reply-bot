(function() {
  var Ban, Bind, CommandManager, EventEmitter, Icommand, escapeRegex,
  ...
  CommandManager = (function(_super) {
    ...
    CommandManager.prototype.handleRaw = function(sender, type, contents, textRouter) {
      var command, event, module, _i, _j, _len, _len1, _ref, _ref1;
      event = {
        cancelled: false
      };
      _ref = this.commands;
    ...
    CommandManager.prototype.handleText = function(sender, text, textRouter, isCommand, fromBinding) {
      ...
      result = {};
      commandManager = this;
      result.isCommand = isCommand || (text.search(escapeRegex(this.identifier))) === 0 || !!(textRouter.getIdentifier ? (text.search(escapeRegex(textRouter.getIdentifier()))) === 0 : void 0);
      result.sender = sender;
      result.text = text;
      result.fromBinding = false;
      if ((this.handleRaw(sender, "before_iscommand", result, textRouter)).cancelled) {
        return false;
      }
      text = result.text;
      ...
      if (this.commandMap[command].hasPermission(sender, text, args, this.storage, textRouter, commandManager, fromBinding)) {
        ...
        if (!this.commandMap[command].handle(sender, text, args, this.storage, textRouter, commandManager, fromBinding)) {
          return this._sendToPlace(textRouter, sender.sender, sender.target, sender.channel, this.commandMap[command].help("" + currentIdentifier + " " + command));
        }
      } else {
    ...
    CommandManager.prototype._commandHelp = function(sender, text, args, storage, textRouter, commandManager) {
      var command, currentIdentifier, index, message, _i, _len, _ref;
      currentIdentifier = this.identifier;
      if (textRouter.getIdentifier) {
        currentIdentifier = textRouter.getIdentifier();
      }
    ...
    CommandManager.prototype.addRouter = function(textRouter) {
      ...
      textRouter.on("input", (function(_this) {
        return function(message, sender, router) {
          ...
          return _this.handleText(sender, message, router);
        };
      })(this));
      textRouter.on("rpl_join", (function(_this) {
