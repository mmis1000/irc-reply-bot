...
function extractPacket(packet){
    ...
    try {
        ...
        if ('object' === typeof result.description) {
            result._description = result.description;
...
function status (ip, port, callback, options) {
    ...
    //some server has problem with old protocol that will cause timeout
    options.tryNewProtocolIfTimeout = options.tryNewProtocolIfTimeout !== undefined ? !!options.tryNewProtocolIfTimeout : true;
    ...
    function send () {
        ...
        function onTimeout () {
            ...
            client.destroy();
            if (!options.tryNewProtocolIfTimeout || self.state == 1) {
                defaultResult.error = new Error('timeout');
                defaultResult.ip = ip;
                defaultResult.port = port;
                callback(defaultResult);
            } else {
                log('try new protocol instead!' + (" $$0 : $$1 ").replace('$$0', ip).replace('$$1', port));
                self.state = 1;
                send();
            }
        }
