  ...
  CommandRegex = (function(_super) {
    ...
    CommandRegex.prototype.handleRaw = function(sender, type, content, textRouter, commandManager) {
      ...
      if (!tags) {
        ...
        result = this._parseCommand(tags[2].text);
        sayer = tags[1].text;
      ...
      if (content.replyTo) {
        if (false !== this._replaceText(content.replyTo.message.text, result)) {
          ...
          if (sender.sender !== content.replyTo.sender.sender) {
            commandManager.send(sender, textRouter, "" + referredBy + " " + this.locale.think + " " + content.replyTo.sender.sender + " " + this.locale.preMean + "\u0002" + this.locale.mean + "\u000f" + this.locale.postMean + " \u001d" + changesMessage);
          } else {
            commandManager.send(sender, textRouter, "" + sayer + " " + this.locale.preMean + "\u0002" + this.locale.mean + "\u000f" + this.locale.postMean + " \u001d" + changesMessage);
          }
        }
        return true;
      }
