(function() {
  ...
  CommandLogs = (function(_super) {
    ...
    function CommandLogs(dbStorage) {
      this.dbStorage = dbStorage;
      this.maxRecord = 20;
      this.defaultPageShow = 10;
      this.userPageShowMax = 15;
    ...
    CommandLogs.prototype._showlog = function(sender, text, args, storage, textRouter, commandManager, list) {
      var date, pageNumber, record, recordsPerPage, replys, result, _i, _len, _ref;
      if (args.length > 4) {
        return false;
      }
    ...
    CommandLogs.prototype._findlog = function(sender, text, args, storage, textRouter, commandManager, list) {
      var pageNumber, recordsPerPage, regex;
    ...
    CommandLogs.prototype._pagelog = function(list, itemPerPage, pageNumber) {
    ...
    CommandLogs.prototype.help = function(commandPrefix) {
      return ["view recent talks, this command will force send to you instead of channel ", "Usage:", "" + commandPrefix + " show [page, default to 1 if omit] [records per page, default to " + this.defaultPageShow + " if omit]", "" + commandPrefix + " find [sender/text] [text] [page, default to 1 if omit] [records per page, default to " + this.defaultPageShow + " if omit]", "" + commandPrefix + " find [date] [yyyy-mm-dd|today] [page, default to 1 if omit] [records per page, default to " + this.defaultPageShow + " if omit]"];
    };
