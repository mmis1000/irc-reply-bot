(function() {
  ...
  CommandTranslate = (function(_super) {
    ...
    CommandTranslate.prototype.handle = function(sender, text, args, storage, textRouter, commandManager) {
      var langPair, message, options, success;
      ...
      request(options, function(error, res, body) {
        if (error) {
          return commandManager.send(sender, textRouter, 'translate: ' + error.toString());
        } else {
          return commandManager.send(sender, textRouter, 'translate: ' + body.responseData.translatedText);
        }
      });
      success = true;
      return success;
